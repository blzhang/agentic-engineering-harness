...
Intermediate Issue work uses one PRD/version batch review budget: at most three isolated reviewer subagent/session passes total, fixing or evidentially rejecting P0/P1 findings after each pass. PRD/version-plan review passes do not count against this implementation budget. The final version PR uses PR-level review when available, and that final review/fix/re-review loop is not limited by the isolated reviewer budget.
